...
styles.add(ParagraphStyle(name="GuideTitle", parent=styles["Title"], fontName="Helvetica-Bold", fontSize=27, leading=31, textColor=GREEN, spaceAfter=8 * mm, alignment=TA_LEFT))
styles.add(ParagraphStyle(name="GuideSubtitle", parent=styles["Normal"], fontName="Helvetica", fontSize=13, leading=18, textColor=INK, spaceAfter=7 * mm))
styles.add(ParagraphStyle(name="SectionHeading", parent=styles["Heading2"], fontName="Helvetica-Bold", fontSize=17, leading=21, textColor=GREEN, spaceBefore=3 * mm, spaceAfter=4 * mm))
styles.add(ParagraphStyle(name="Subheading", parent=styles["Heading3"], fontName="Helvetica-Bold", fontSize=11.5, leading=15, textColor=GREEN_2, spaceAfter=1.5 * mm))
styles.add(ParagraphStyle(name="BodyGuide", parent=styles["BodyText"], fontName="Helvetica", fontSize=10, leading=14.2, textColor=INK, spaceAfter=3.5 * mm))
styles.add(ParagraphStyle(name="SmallGuide", parent=styles["BodyText"], fontName="Helvetica", fontSize=8.5, leading=11.5, textColor=MUTED))
styles.add(ParagraphStyle(name="DraftNotice", parent=styles["BodyText"], fontName="Helvetica-Bold", fontSize=9.2, leading=13, textColor=ERROR))
styles.add(ParagraphStyle(name="Checklist", parent=styles["BodyText"], fontName="Helvetica", fontSize=8.7, leading=11.5, textColor=INK))
styles.add(ParagraphStyle(name="TableHead", parent=styles["BodyText"], fontName="Helvetica-Bold", fontSize=8.2, leading=10, textColor=colors.white, alignment=TA_CENTER))
styles.add(ParagraphStyle(name="BulletGuide", parent=styles["BodyText"], fontName="Helvetica", fontSize=10, leading=14, leftIndent=5 * mm, firstLineIndent=-3.5 * mm, bulletIndent=1.5 * mm, textColor=INK, spaceAfter=2.2 * mm))

def para(text: str, style: str = "BodyGuide") -> Paragraph:
    return Paragraph(text, styles[style])

def callout(text: str, background=PALE, color=INK) -> Table:
    content = Paragraph(text, ParagraphStyle(name=f"Callout{abs(hash(text))}", parent=styles["BodyGuide"], fontName="Helvetica", fontSize=9.4, leading=13.2, textColor=color, spaceAfter=0))
...
def make_document() -> None:
    ...
    table = Table(rows, colWidths=[48 * mm, 114 * mm])
    ...
    checklist = Table(table_data, colWidths=[105 * mm, 18.5 * mm, 22 * mm, 16.5 * mm], repeatRows=1)
